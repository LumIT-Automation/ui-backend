...
printf "$wallBanner Internal database configuration...\n"
    if podman exec ui-backend mysql -e "exit"; then
        ...
        if [ "$(podman exec ui-backend mysql --vertical -e "SELECT User FROM mysql.user WHERE User = 'uib';" | tail -1 | awk '{print $2}')" == "" ]; then
            ...
            podman exec ui-backend mysql -e 'GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, INDEX, ALTER, CREATE TEMPORARY TABLES, CREATE VIEW, SHOW VIEW, EXECUTE ON `api`.* TO `uib`@`localhost`;'
        else
        ...
        if [ "$(podman exec ui-backend mysql --vertical -e "SHOW DATABASES LIKE 'uib';" | tail -1 | awk -F': ' '{print $2}')" == "" ]; then
            ...
            podman exec ui-backend mysql api -e "source /var/www/ui-backend/ui_backend/sql/uib.schema.sql" # restore database schema.
            podman exec ui-backend mysql api -e "source /var/www/ui-backend/ui_backend/sql/uib.data.sql" # restore database data.
        fi
